...
const UsersTable = ({ users, onDelete, onStatusChange, onEdit }) => {
  ...
  const handleDelete = (user) => {
    ...
    showConfirm({
      ...
      message: `¿Estás seguro de que deseas eliminar permanentemente a ${nombreParaMostrar}? Esta acción no se puede deshacer.`,
      confirmText: "Sí, Eliminar",
      confirmColor: "bg-red-600 hover:bg-red-700",
      onConfirm: () => {
        if (onDelete) onDelete(user._id || user.id);
      },
    });
  };
  ...
  const handleStatus = (user) => {
    ...
    showConfirm({
      ...
      message: `¿Confirmas que deseas ${nuevaAccion.toLowerCase()} el acceso de ${nombreParaMostrar}?`,
      confirmText: `Sí, ${nuevaAccion}`,
      confirmColor: esActivo
        ? "bg-amber-600 hover:bg-amber-700"
        : "bg-green-600 hover:bg-green-700",
      onConfirm: () => {
        if (onStatusChange) onStatusChange(user._id || user.id, nuevaAccion);
      },
    });
  };
